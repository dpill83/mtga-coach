#!/usr/bin/env python3
"""
MTGA Coach - Main Parser Application with State Management

Enhanced main application that integrates the parser with state management.
This is the updated version that includes Phase 2 functionality.
"""
...
class MTGACoachWithState:
    """Enhanced MTGA Coach application with state management."""
    ...
    async def initialize(self) -> bool:
        ...
        try:
            logger.info("Initializing MTGA Coach with State Management...")
    ...
    async def start(self) -> bool:
        """Start the MTGA Coach with state management."""
        try:
            ...
            logger.info(f"MTGA Coach with State Management started successfully")
    ...
    def get_state_statistics(self):
        ...
        if self.state_integration:
            return self.state_integration.get_state_statistics()
        return None
    
    def validate_state(self) -> list:
        """Validate the current game state."""
        if self.state_integration:
            return self.state_integration.validate_state()
        return []
    
    def clear_persisted_state(self) -> bool:
        """Clear persisted state."""
        if self.state_integration:
            return self.state_integration.clear_persisted_state()
        return False

async def main():
    """Main function."""
    # Create coach instance
    coach = MTGACoachWithState()
...
def run_state_test():
    """Run a test with state management."""
    print("MTGA Coach - State Management Test")
    print("=" * 40)
    ...
    # Process events through state manager
    for event in events:
        success = state_manager.process_event(event)
        print(f"  Processed {event.event_type}: {success}")
    
    # Get state summary
    summary = state_manager.get_game_summary()
    print(f"\nGame state summary:")
    print(f"  Status: {summary['status']}")
    print(f"  Turn: {summary['turn_number']}")
    print(f"  Phase: {summary['current_phase']}")
    
    if summary['self_player']:
        print(f"  Self player life: {summary['self_player']['life_total']}")
        print(f"  Self player hand size: {summary['self_player']['hand_size']}")
    
    if summary['opponent_player']:
        print(f"  Opponent player life: {summary['opponent_player']['life_total']}")
        print(f"  Opponent player hand size: {summary['opponent_player']['hand_size']}")

if __name__ == "__main__":
    import os
    
    # Check if we should run state test
